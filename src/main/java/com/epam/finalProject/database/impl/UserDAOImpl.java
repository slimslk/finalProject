...
public class UserDAOImpl implements UserDAO {
    ...
    public void insertUser(User user, String roleName, String statusName) throws AppException {
        DBManager dbManager = DBManager.getInstance();
        Connection connection = null;
        int roleId = new RoleDAOImpl().getRoleByName(roleName);
        int statusId = new UserStatusDAOImpl().getStatusByName(statusName);
        int i = 0;
        try {
            connection = dbManager.getConnection();
    ...
    private User mapRow(ResultSet rs) {
        try {
            ...
            user.setUserInfo(new UserInfoDAOImpl().getUserInfoById(user.getId()));
            return user;
